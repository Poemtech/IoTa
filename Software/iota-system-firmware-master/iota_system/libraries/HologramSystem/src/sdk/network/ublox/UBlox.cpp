...
// Quick macro defs to output the APN at compile-time
#define STR(x) #x
#define XSTR(x) STR(x)
#pragma message("APN=" XSTR(APN_NAME))

        if(rn4_series) {
            modem->set("+CEREG", "2");
            modem->set("+CGDCONT", "1"); // Clear any existing APN
            modem->set("+CGDCONT", "1,\"IP\"," APN_NAME ",\"0.0.0.0\",0,0"); // Set the compile-time APN
        } else {
            modem->set("+CREG", "2");
            // UPSD params are not stored in NVRAM unless +UPSDA 0,1 is issued and it is not in this code base.
            modem->set("+UPSD", "0,1,\"" APN_NAME "\"", 3000); // Set the compile-time APN
